...
struct CreateRelRangeIndexBindData final : TableFuncBindData {
    ...
    CreateRelRangeIndexBindData(std::string tableName, table_id_t relGroupID,
    ...
    std::unique_ptr<TableFuncBindData> copy() const override {
        return std::make_unique<CreateRelRangeIndexBindData>(
...
static std::unique_ptr<TableFuncBindData> bindFunc(main::ClientContext* context,
    ...
    if (!context->getTransactionContext()->isAutoTransaction()) {
        throw BinderException(
            "CREATE_REL_RANGE_INDEX is only supported in auto transaction mode.");
    }
...
static offset_t tableFunc(const TableFuncInput& input, TableFuncOutput&) {
    auto& bindData = *input.bindData->constPtrCast<CreateRelRangeIndexBindData>();
    ...
    storage::IndexInfo indexInfo{bindData.propertyName, rangeType.typeName,
    ...
    auto storageInfoPtr =
        std::make_unique<SecondaryRangeIndexStorageInfo>(0, bindData.columnID);
    ...
    auto indexEntry = std::make_unique<IndexCatalogEntry>(
        rangeType.typeName, bindData.relGroupID, bindData.propertyName,
...
function_set CreateRelRangeIndexFunction::getFunctionSet() {
