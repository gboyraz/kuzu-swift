...
struct ListUniqueIndexesBindData final : TableFuncBindData {
    std::vector<std::string> propertyNames;

    ListUniqueIndexesBindData(binder::expression_vector columns,
        std::vector<std::string> propertyNames)
        : TableFuncBindData{std::move(columns), propertyNames.size()},
          propertyNames{std::move(propertyNames)} {}

    std::unique_ptr<TableFuncBindData> copy() const override {
        return std::make_unique<ListUniqueIndexesBindData>(columns, propertyNames);
    }
};
...
static std::unique_ptr<TableFuncBindData> bindFunc(main::ClientContext* context,
    ...
    for (auto* indexEntry : indexEntries) {
        if (indexEntry->getIndexType() == "UNIQUE_HASH") {
    ...
    return std::make_unique<ListUniqueIndexesBindData>(
        std::move(columns), std::move(propertyNames));
}

static offset_t internalTableFunc(const TableFuncMorsel& morsel, const TableFuncInput& input,
    DataChunk& output) {
    auto& bindData = *input.bindData->constPtrCast<ListUniqueIndexesBindData>();
...
function_set ListUniqueIndexesFunction::getFunctionSet() {
