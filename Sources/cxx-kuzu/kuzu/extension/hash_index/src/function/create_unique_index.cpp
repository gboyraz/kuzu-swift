...
struct CreateUniqueIndexBindData final : TableFuncBindData {
    ...
    CreateUniqueIndexBindData(std::string tableName, table_id_t tableID, std::string propertyName,
    ...
    std::unique_ptr<TableFuncBindData> copy() const override {
        return std::make_unique<CreateUniqueIndexBindData>(
...
static void validateKeyType(PhysicalTypeID type) {
    switch (type) {
    case PhysicalTypeID::INT8: case PhysicalTypeID::INT16: case PhysicalTypeID::INT32:
    case PhysicalTypeID::INT64: case PhysicalTypeID::INT128:
    case PhysicalTypeID::UINT8: case PhysicalTypeID::UINT16: case PhysicalTypeID::UINT32:
    case PhysicalTypeID::UINT64:
    case PhysicalTypeID::FLOAT: case PhysicalTypeID::DOUBLE:
    case PhysicalTypeID::STRING: case PhysicalTypeID::BOOL:
        return;
    default:
        throw BinderException(
            stringFormat("Unique index does not support type {}.", PhysicalTypeUtils::toString(type)));
    }
}
...
static std::unique_ptr<TableFuncBindData> bindFunc(main::ClientContext* context,
    ...
    if (!context->getTransactionContext()->isAutoTransaction()) {
        throw BinderException("CREATE_UNIQUE_INDEX is only supported in auto transaction mode.");
    }
...
static offset_t tableFunc(const TableFuncInput& input, TableFuncOutput&) {
    auto& bindData = *input.bindData->constPtrCast<CreateUniqueIndexBindData>();
    ...
    auto uniqueType = SecondaryHashIndex::getUniqueIndexType();
    storage::IndexInfo indexInfo{bindData.propertyName, uniqueType.typeName, bindData.tableID,
        {bindData.columnID}, {bindData.keyType},
        false /*isPrimary*/, false /*isBuiltin*/};

    auto storageInfoPtr = std::make_unique<SecondaryHashIndexStorageInfo>(
        0, bindData.columnID, true /*isUnique*/);
    auto index = std::make_unique<SecondaryHashIndex>(
        std::move(indexInfo), std::move(storageInfoPtr));

    // Scan existing data and populate index (with duplicate checking)
    ...
    auto indexEntry = std::make_unique<IndexCatalogEntry>(
        uniqueType.typeName, bindData.tableID, bindData.propertyName,
        std::vector<property_id_t>{}, std::make_unique<HashIndexAuxInfo>(true));
    catalog->createIndex(transaction, std::move(indexEntry));
    nodeTable.addIndex(std::move(index));
    return 0;
}

function_set CreateUniqueIndexFunction::getFunctionSet() {
