...
struct DropHashIndexBindData final : TableFuncBindData {
    table_id_t tableID;
    std::string tableName;
    std::string indexName;

    DropHashIndexBindData(table_id_t tableID, std::string tableName, std::string indexName)
        : TableFuncBindData{0}, tableID{tableID}, tableName{std::move(tableName)},
          indexName{std::move(indexName)} {}

    std::unique_ptr<TableFuncBindData> copy() const override {
        return std::make_unique<DropHashIndexBindData>(tableID, tableName, indexName);
    }
};
...
static std::unique_ptr<TableFuncBindData> bindFunc(main::ClientContext* context,
    ...
    if (!context->getTransactionContext()->isAutoTransaction()) {
        throw BinderException("DROP_HASH_INDEX is only supported in auto transaction mode.");
    }
...
static offset_t tableFunc(const TableFuncInput& input, TableFuncOutput&) {
    auto& bindData = *input.bindData->constPtrCast<DropHashIndexBindData>();
    auto clientContext = input.context->clientContext;
    auto storageManager = clientContext->getStorageManager();
    auto& nodeTable =
        storageManager->getTable(bindData.tableID)->cast<storage::NodeTable>();
    // Drop from node table storage
...
function_set DropHashIndexFunction::getFunctionSet() {
