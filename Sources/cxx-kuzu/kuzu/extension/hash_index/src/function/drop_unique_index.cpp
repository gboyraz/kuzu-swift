...
struct DropUniqueIndexBindData final : TableFuncBindData {
    table_id_t tableID;
    std::string tableName;
    std::string indexName;

    DropUniqueIndexBindData(table_id_t tableID, std::string tableName, std::string indexName)
        : TableFuncBindData{0}, tableID{tableID}, tableName{std::move(tableName)},
          indexName{std::move(indexName)} {}

    std::unique_ptr<TableFuncBindData> copy() const override {
        return std::make_unique<DropUniqueIndexBindData>(tableID, tableName, indexName);
    }
};
...
static std::unique_ptr<TableFuncBindData> bindFunc(main::ClientContext* context,
    ...
    if (!context->getTransactionContext()->isAutoTransaction()) {
        throw BinderException("DROP_UNIQUE_INDEX is only supported in auto transaction mode.");
    }
...
static offset_t tableFunc(const TableFuncInput& input, TableFuncOutput&) {
    auto& bindData = *input.bindData->constPtrCast<DropUniqueIndexBindData>();
...
function_set DropUniqueIndexFunction::getFunctionSet() {
