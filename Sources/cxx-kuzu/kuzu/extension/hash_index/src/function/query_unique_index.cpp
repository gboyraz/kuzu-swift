...
struct QueryUniqueIndexBindData final : TableFuncBindData {
    table_id_t tableID;
    std::string indexName;
    std::vector<offset_t> resultOffsets;

    QueryUniqueIndexBindData(binder::expression_vector columns, table_id_t tableID,
    ...
    std::unique_ptr<TableFuncBindData> copy() const override {
        return std::make_unique<QueryUniqueIndexBindData>(
...
static std::unique_ptr<TableFuncBindData> bindFunc(main::ClientContext* context,
    ...
    TypeUtils::visit(
        ...
        [&](auto) {
            throw BinderException("Unsupported key type for unique index query.");
        });
...
static offset_t internalTableFunc(const TableFuncMorsel& morsel, const TableFuncInput& input,
    DataChunk& output) {
    auto& bindData = *input.bindData->constPtrCast<QueryUniqueIndexBindData>();
...
function_set QueryUniqueIndexFunction::getFunctionSet() {
